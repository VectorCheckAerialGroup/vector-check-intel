...
if data and "hourly" in data:
    ...
    # Dynamically grab the unit string straight from the API, defaulting to 'km/h' if missing.
    ...
    # Dynamically inject the native unit into the UI
    ...
    for alt in [400, 300, 200, 100]:
        ...
        # Dynamically label the dataframe columns
    ...
    for alt in [5000, 4000, 3000, 2000, 1000]:
        ...
        # Dynamically label the extended dataframe columns
        stack_ext.append({
            "Alt (AGL)": f"{alt}ft", 
            "Dir": f"{int(d_e):03d}°", 
    ...
    csv_header = (
        ...
        f"Wind Unit Standard: {raw_wind_unit}\n"  # Added explicit unit callout to the CSV header
