...
def get_turb_ice(alt, wind_spd, sfc_spd, gust, wx, is_stable, icing_cond, t_temp, rh, terrain_type="Land"):
    ...
    if alt > 0:
        ...
        if alt_temp > 0 or alt_temp < -40:
            ...
            # 1. Active Weather Overrides (Table 1, 3, & Image 3)
            if wx_val in [66, 67]: # FZRA
                ice = "SEV CLR"
            elif wx_val in [95, 96, 99]: # TS / CB
                ice = "SEV CLR"
            elif wx_val in [56, 57, 77]: # FZDZ, SG
                ice = "MOD MX"
            elif wx_val in [80, 81, 82, 85, 86]: # Showers (Convective)
                ice = "MOD MX"
                
            # 2. Stratiform / Cloud Mass Evaluation (Table 2 & Text)
            elif rh_val >= 80: # Primary trigger for visible moisture
                # Worst icing zone
                if 0 >= alt_temp >= -15:
                    if rh_val >= 90:
                        ice = "MOD RIME" # Proxy for thickness > 5000ft
                    else:
                        ice = "LGT RIME"
                # Decreasing LWC zone
                elif -15 > alt_temp >= -20:
